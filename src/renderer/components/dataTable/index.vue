    ...
    <div :class="s.dataTable" v-loading="loading">
        <template v-if="!loading && data.length">
            <div :class="s.top" v-if="canSearch">
                <p :class="s.total">共&nbsp;<span>{{total}}</span>&nbsp;首歌曲</p>
            ...
            <el-row v-if="pagination" style="text-align: center">
                <el-pagination @current-change="handleCurrentChange" :current-page="pageNo" :page-size="pageSize" layout="total, prev, pager, next, jumper" :total="total"/>
            </el-row>
        </template>
