...
export default {
    ...
    computed: {
        ...
        qualityText() {
            let qualities = this.qualities.filter(item => item.checked)
            if (qualities && qualities.length > 0) {
                return qualities[0].name
            } else {
                return ""
            }
        },
        ...
        url(val) {
            if (val) {
                this.qualities = this.qualities.map((item, i) => {
                    item.checked = item.brRange[0] <= this.quality && item.brRange[1] >= this.quality;
                    const quality = this.info.quality
                    const vendor = this.info.vendor
                    if (quality) {
                        switch (i) {
                            case 0:
                                item.disabled = !quality['128'] && !vendor
                                break
                            case 1:
                                item.disabled = !quality['192'] || vendor === 'qq'
                                break
                            case 2:
                                item.disabled = !quality['320'] || vendor === 'qq'
                                break
                            case 3:
                                item.disabled = !quality['999'] || vendor === 'qq'
                                break
                        }
                    } else {
                        item.disabled = true
